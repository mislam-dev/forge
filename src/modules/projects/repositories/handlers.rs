...
pub async fn connect_repository(
    ...
    let repository = ProjectRepositoriesService::connect_repository(&state.db, claims.sub, is_admin, id, payload).await?;
...
pub async fn get_repository(
    ...
    let is_admin = claims.role.iter().any(|r| r.eq_ignore_ascii_case("admin") || r.eq_ignore_ascii_case("system_admin"));
    let repository = ProjectRepositoriesService::get_repository(&state.db, claims.sub, is_admin, id).await?;
...
pub async fn update_repository(
    ...
    let is_admin = claims.role.iter().any(|r| r.eq_ignore_ascii_case("admin") || r.eq_ignore_ascii_case("system_admin"));
    let repository = ProjectRepositoriesService::update_repository(&state.db, claims.sub, is_admin, id, payload).await?;
