...
#[tokio::test]
async fn test_system_dashboard_unauthorized_without_jwt() {
...
#[tokio::test]
async fn test_user_dashboard_unauthorized_without_jwt() {
    let config = setup_test_config();
    let state = AppState::mock(config);
    let app = create_app(state).await.expect("App creation failed");

    let req = Request::builder()
        .uri("/api/dashboard/user")
        .method("GET")
        .body(Body::empty())
        .unwrap();
...
#[tokio::test]
async fn test_org_dashboard_unauthorized_without_jwt() {
    let config = setup_test_config();
    let state = AppState::mock(config);
    let app = create_app(state).await.expect("App creation failed");

    let req = Request::builder()
        .uri(format!("/api/dashboard/org/{}", Uuid::new_v4()))
        .method("GET")
        .body(Body::empty())
        .unwrap();
