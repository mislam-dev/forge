...
- **2026-08-19:** Implemented Projects and Submodules (`src/modules/projects/`) with submodules `projects`, `repositories`, `environment_variables`, `assignments`, and `permissions`. Added support for Dual Workspaces (Personal Projects without an organization and Organization Projects), encrypted PAT token storage masked as `"••••••••"`, POSIX key validation with secret value masking, atomic bulk environment variable creation inside DB transactions, project role resolution (`Owner > Admin > Developer > Viewer`), and 10/10 passing integration tests in `tests/projects_tests.rs`.
